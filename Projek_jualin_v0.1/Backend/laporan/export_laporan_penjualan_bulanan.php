<?php
include "../koneksi.php";
require "../fpdf/fpdf.php"; // ✅ path sesuai: Backend/fpdf/fpdf.php
...
if ($type === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=laporan_penjualan_bulanan_" . date("Ymd") . ".xls");
    header("Pragma: no-cache");

    echo "\xEF\xBB\xBF"; // BOM UTF-8

    echo "LAPORAN PENJUALAN PRODUK BULANAN\n";
    echo "Toko\t: " . $nama_toko . "\n";
    echo "Dicetak\t: " . date("d/m/Y H:i") . "\n\n";

    echo "Bulan\tTotal Transaksi\tTotal Produk Terjual\tTotal Penjualan\n";
    foreach ($data as $row) {
        echo fmtBulan($row['BULAN']) . "\t"
            . $row['TOTAL_TRANSAKSI'] . "\t"
...
// Custom class untuk header & footer
...
$pdf = new PDF('P', 'mm', 'A4');
$pdf->judulLaporan = 'Laporan Penjualan Produk Bulanan';
...
// ----- RINGKASAN STAT -----
// Gambar 3 kotak stat berjajar: gunakan Cell dengan fill agar posisi teks terjaga
$colW = (210 - 28) / 3; // lebar tiap kotak, margin 14 kiri+kanan

$statData = [
    ['label' => 'Total Bulan Data',     'value' => count($data) . ' Bulan',               'color' => [30,136,229]],
...
foreach ($statData as $i => $s) {
    ...
    // Gambar kotak background dengan Rect (4 param + style — benar untuk FPDF)
    $pdf->SetFillColor($r, $g, $b);
    $pdf->Rect($xBox, $yStatStart, $colW - 3, 18, 'F'); // ✅ tanpa parameter ekstra
...
$pdf->Rect(14, $yS, 182, 16, 'F'); // ✅ tanpa parameter ekstra
...
$cols = [
    ...
    ['label' => 'Total Transaksi',     'w' => 40, 'align' => 'C'],
    ['label' => 'Produk Terjual',      'w' => 40, 'align' => 'C'],
    ['label' => 'Total Penjualan',     'w' => 50, 'align' => 'R'],
];
...
foreach ($data as $row) {
    ...
    $pdf->Cell($cols[1]['w'], 7, fmtBulan($row['BULAN']),                  0, 0, 'L', true);
...
// ===== OUTPUT =====
$filename = "laporan_penjualan_bulanan_" . date("Ymd_His") . ".pdf";
