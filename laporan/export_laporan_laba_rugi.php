...
$sql = "SELECT
            ...
            COALESCE(SUM(ti.JUMLAH), 0) AS total_terjual,
        ...
        ORDER BY COALESCE(p.NAMA_PRODUK, ti.NAMA_PRODUK) ASC";
...
if ($type === 'excel') {
    ...
    header('Content-Disposition: attachment; filename="laporan_laba_rugi_' . date("Y-m-d") . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['PRODUK','TOTAL TERJUAL','TOTAL PENJUALAN (Rp)','TOTAL MODAL (Rp)','LABA (Rp)']);
    foreach ($rows as $r) {
        fputcsv($out, [
            $r['NAMA_PRODUK'], $r['total_terjual'],
...
if ($type === 'pdf') {
    ...
    foreach (['Produk','Terjual','Total Penjualan','Total Modal','Laba'] as $i=>$h)
        $pdf->Cell($w[$i],8,$h,1,0,'C');
    $pdf->Ln();

    $pdf->SetFont('Arial','',9);
    foreach ($rows as $r) {
        $name = mb_strlen($r['NAMA_PRODUK'])>45 ? mb_substr($r['NAMA_PRODUK'],0,42).'...' : $r['NAMA_PRODUK'];
        $pdf->Cell($w[0],7,$name,1,0);
        $pdf->Cell($w[1],7,$r['total_terjual'],1,0,'C');
        $pdf->Cell($w[2],7,'Rp '.number_format($r['total_penjualan'],0,',','.'),1,0,'R');
        $pdf->Cell($w[3],7,'Rp '.number_format($r['total_modal'],0,',','.'),1,0,'R');
        $pdf->Cell($w[4],7,'Rp '.number_format($r['laba'],0,',','.'),1,1,'R');
    }
    $pdf->Output('I','laporan_laba_rugi.pdf',true); exit;
}
