...
$sql .= " GROUP BY DATE(t.TANGGAL) ORDER BY tanggal ASC";
...
function fmtTgl($str) {
    ...
    if (!$d) return $str;
    return $d->format('d') . ' ' . $names[intval($d->format('m')) - 1] . ' ' . $d->format('Y');
}
...
if ($type === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=laporan_laba_rugi_harian_" . date("Ymd") . ".xls");
    header("Pragma: no-cache");
    echo "\xEF\xBB\xBF";

    echo "LAPORAN LABA RUGI HARIAN\n";
    echo "Toko\t: " . $nama_toko . "\n";
    echo "Dicetak\t: " . date("d/m/Y H:i") . "\n";
    if ($start || $end) {
        echo "Filter\t: " . ($start ? $start : '-') . " s/d " . ($end ? $end : '-') . "\n";
    }
    echo "Total Hari\t: " . count($rows) . " hari\n\n";

    echo "Tanggal\tTotal Penjualan (Rp)\tTotal Modal (Rp)\tLaba / Rugi (Rp)\tStatus\n";
    foreach ($rows as $r) {
        $status = ($r['laba'] >= 0) ? "UNTUNG" : "RUGI";
        echo fmtTgl($r['tanggal']) . "\t"
...
$pdf = new PDF('P', 'mm', 'A4');
$pdf->judulLaporan = 'Laporan Laba Rugi Harian';
...
if ($start || $end) {
    $pdf->Cell(0, 5, 'Filter: ' . ($start ? $start : '-') . ' s/d ' . ($end ? $end : '-'), 0, 1, 'R');
}
$pdf->Ln(2);
...
$statData = [
    ...
    ['label' => 'Total Hari',      'value' => count($rows) . ' hari', 'color' => [25,  118, 210]],
];
...
$yS = $pdf->GetY();
$pdf->SetFillColor($isRugiTotal ? 198 : 123, $isRugiTotal ? 40 : 31, $isRugiTotal ? 40 : 162);
...
// ===== HEADER TABEL — 8+40+46+46+36+6 = 182mm =====
$cols = [
    ['label' => 'No',              'w' =>  8, 'align' => 'C'],
    ['label' => 'Tanggal',         'w' => 40, 'align' => 'L'],
    ['label' => 'Total Penjualan', 'w' => 46, 'align' => 'R'],
    ['label' => 'Total Modal',     'w' => 46, 'align' => 'R'],
    ['label' => 'Laba / Rugi',     'w' => 36, 'align' => 'R'],
    ['label' => 'Sts',             'w' =>  6, 'align' => 'C'],
];
...
// ===== BARIS TOTAL =====
$pdf->SetFillColor($isRugiTotal ? 198 : 56, $isRugiTotal ? 40 : 142, $isRugiTotal ? 40 : 60);
$pdf->SetTextColor(255, 255, 255);
$pdf->SetFont('Arial', 'B', 9);
...
$pdf->SetTextColor(100, 100, 100);
$pdf->Cell(0, 5, 'Keterangan: Baris merah = hari rugi  |  ^ = untung  |  v = rugi', 0, 1, 'L');

$pdf->Output('D', 'laporan_laba_rugi_harian_' . date('Ymd') . '.pdf');
exit;
